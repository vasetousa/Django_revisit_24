...
class AlbumCreateView(CreateView):
    ...
    def form_valid(self, form):
        profile = Profile.objects.first()  # Get the profile associated with the user
        form.instance.user = profile  # Set the profile as the user of the album
        return super().form_valid(form)
...
def delete_related_albums(sender, instance, **kwargs):
    try:
        # Retrieve all associated albums
        profile = Profile.objects.first()
        albums = profile.albums.all()

        # Delete each associated album
        for album in albums:
            album.delete()
    except Album.DoesNotExist:
        # If no associated albums exist, do nothing
        pass
